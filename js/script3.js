...
document.getElementById("nb-color").
    addEventListener("change", function(){
        change();
        makeColor();
    }
...
function makeColor(){
    let nb = document.getElementById("nb-color").value;
    let index = 0;
    let boxes = document.querySelectorAll('.box');
    ...
    for(let box of boxes){
        box.style.borderColor = colors[index].value;

        //iterate through collors
        console.log("nb: "+nb);
        console.log("index: "+index);
        index++;
        if(index> (nb-1)){index = 0;}
    }
}